...
if __name__ == "__main__":
    ...
    def _google_search_only_for_vertexai():
        if _PROVIDER_CONFIG["vertexai"]:
            return [
                types.Tool(url_context=types.UrlContext()),
                types.Tool(google_search=types.GoogleSearch()),
            ]
        else:
            print("⛔️ NOT Using Vertex AI, NO buildin Search API")
            # google.genai.errors.ClientError: 400 INVALID_ARGUMENT. {'error': {'code': 400, 'message': 'Tool use with function calling is unsupported by the model.', 'status': 'INVALID_ARGUMENT'}}
            # So, the aistudio key approach is atm not working.
            return []
